...
function BrowserStream({
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // If VNC is actively connected, the stream is working —
        // don't let REST polling kill it. The backend VNC verify loop
        // will close the WebSocket when the session truly ends.
        if (isVncConnectedRef.current) {
  ...
  const isVncConnectedRef = useRef(false);
  ...
  // keep ref in sync so polling callback always has current VNC state
  useEffect(() => {
    isVncConnectedRef.current = isVncConnected;
  }, [isVncConnected]);

  // browser is ready — VNC connection is the source of truth, not REST polling
  useEffect(() => {
    setIsReady(isVncConnected && isCanvasReady);
  }, [isCanvasReady, isVncConnected]);
