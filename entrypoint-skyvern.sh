...
# Start VNC streaming services (x11vnc + websockify) for live view + Take Control
echo "Starting VNC streaming services..."
x11vnc -display :99 -bg -nopw -listen 0.0.0.0 -xkb -forever -rfbport 5900 > /dev/null 2>&1 || echo "Warning: x11vnc failed to start"
websockify 6080 localhost:5900 --daemon > /dev/null 2>&1 || echo "Warning: websockify failed to start"
echo "VNC streaming ready on port 6080"
